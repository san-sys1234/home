/* Unser Zuhause – V149 · intelligenter Haushaltsplaner */
const STORAGE="unser-zuhause-v149";
...
function plannedDateForTask(x){
 ...
 if(d)return d;
 // Defensive UI fallback: an active task is never presented as unplanned.
 const due=nextDue(x,today);
 for(let delta=0;delta<=30;delta++){
   for(const sign of [0,1,-1]){
     const dd=addDays(due,delta*sign);
     if(Math.abs(delta*sign)>30)continue;
