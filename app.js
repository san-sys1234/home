...
const STORAGE="unser-zuhause-v112";
...
function loadState(){
 ...
 s.todayExtras=Array.isArray(s.todayExtras)?s.todayExtras:[];s.completedDays=s.completedDays||{};
...
function plannerKey(){return "v112|"+String(state.__planRevision||0)+"|"+CATALOG.length+"|"+Object.keys(state.lastDone||{}).length+"|"+Object.keys(state.catalogDeleted||{}).length+"|"+state.custom.length}
...
function showEnergy(){const main=document.getElementById("main");let box=document.getElementById("energyBox");if(!box){box=document.createElement("div");box.id="energyBox";box.className="card";main.insertBefore(box,main.children[1]||null)}const day=dayKey();if(state.energySkipDay!==day){state.energySkipDay=day;state.energySkipped=[]}const base=CATALOG.filter(x=>x.area!=="Alltag"&&!x.window&&!isDone(x)&&!isPostponed(x)&&!recent(x,today,7));let candidates=base.filter(x=>!state.energySkipped.includes(x.id)).sort((a,b)=>nextDue(a)-nextDue(b)).slice(0,3);if(candidates.length<3&&base.length>=3){state.energySkipped=[];candidates=base.sort((a,b)=>nextDue(a)-nextDue(b)).slice(0,3)}box.innerHTML=`<div class="topline"><div><b>⚡ Ich habe Energie</b><div class="small">Nur wenn du möchtest – diese Aufgaben werden heute zusätzlich vorgezogen.</div></div><button class="btn" id="energyOther">↻ Andere 3</button></div>`;if(!candidates.length)box.innerHTML+=`<div class="empty">Gerade gibt es keine sinnvolle Zusatzaufgabe. 🥰</div>`;candidates.forEach(x=>{const r=document.createElement("div");r.className="result";r.innerHTML=`<div class="resultText"><b>${esc(x.text)}</b><div class="meta">${esc(x.room)} · regulär ${esc(nextDueLabel(x))}</div></div><button class="btn primary">Heute vorziehen</button>`;r.querySelector("button").onclick=()=>{state.todayExtras.push({id:`extra|${day}|${uid()}`,date:day,text:x.text,room:x.room,area:x.area,description:x.description,source:"extra"});save();render();showEnergy()};box.appendChild(r)});box.querySelector("#energyOther").onclick=()=>{const ids=candidates.map(x=>x.id);state.energySkipped=[...(state.energySkipped||[]),...ids];save();showEnergy()}}
